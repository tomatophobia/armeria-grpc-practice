...
final class EchoClient {
    ...
    EchoClient(int port, SessionProtocol protocol) {
        blockingStub = GrpcClients.builder(protocol.uriText() + "://127.0.0.1:" + port)
                                  .build(EchoBlockingStub.class);
        asyncStub = GrpcClients.builder(protocol.uriText() + "://127.0.0.1:" + port)
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        final EchoClient client = new EchoClient(port, SessionProtocol.H1C);
        List<Message> messages = new ArrayList<>();
        for (int i = 1; i < 4; i++) {
            messages.add(Message.newBuilder().setSeq(i).setTitle("title" + i).setContent("content" + i)
                                .build());
        }
